let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  conn.sendButtonImg(m.chat, json.url, `Nihh @${m.sender.split('@')[0]} jangan sagne ya!`, wm, `Next`, `${usedPrefix}${command}`, m, {mentions: [m.sender], jpegThumbnail: await(await fetch(json.url)).buffer() })
  } catch {
    //m.reply(eror)
  }
}

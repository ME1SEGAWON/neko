let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.send2Button(m.chat, pos, wm, 'tembak', 'tembak', m)
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.send2Button(m.chat, author, pos, wm, 'tembak', 'tembak', m)
    return conn.send2Button(m.chat, pos, wm, '←', 'kiri', '→', 'kanan', m)

...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  ...
  try {
    ...
    let { premium, premiumTime } = db.data.users[m.sender]
...
✾ Nama    : *${name}*
✾ Limit   : *${limit}*
✾ Status  : *${owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender) ? 'Owner 🗿' : 'home ⚔️'}*
✾ Premium : *${owner.map(v => v.replace(/[^0-9]/g, '') + '@s.whatsapp.net').includes(m.sender) ? '🗿' : premium ? `${conn.msToDate(premiumTime - new Date() * 1)}` : 'Gratisan'}*
...
❏ 𝙉𝘼𝙏𝘼𝙇
_*${ahari} Hari ${ajam} Jam ${anet} Menit ${aetek} Detik*_
...
${petik}Note: Jika ada Fitur yg Error Lapor ke owner${petik}`,
      footer: wm,
      title: judul,
      buttonText: "Klik Disini",
      sections
    }
    return conn.sendMessage(m.chat, listMessage, { quoted: ftoko, mentions: await conn.parseMention(judul), contextInfo: { forwardingScore: 99999, isForwarded: true }})
    ...
    await conn.send3TemplateButtonImg(m.chat, await genProfile(conn, m), text.trim(), wm, `🏅Owner`, `${_p}owner`, `🎖ThanksTo`, `${_p}tqto`, `🎗  Donasi  🎗`, `${_p}donasi`, fgclink, { contextInfo: { externalAdReply :{
